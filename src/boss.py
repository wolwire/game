"""Bosses of Meridian City. Each has telegraphed attacks and a second phase.

- warden:    huge riot officer — combos, charges, ground slams
- chorister: ranged hymn-singer — radial bolt bursts, scream AOE, summons
- hound:     Patient Zero — fast circling lunges
- archivist: final boss — bolts, teleports, 'rewrite' delayed AOEs, drones
"""
import math
import random
...
BOSS_STATS = {
    ...
    'warden':    (640, 2.1, 30, 900, 'boss_warden'),
    'chorister': (520, 1.4, 22, 850, 'boss_chorister'),
    'hound':     (400, 4.6, 18, 600, 'feral'),
    'archivist': (860, 2.2, 26, 0, 'boss_archivist'),
}
...
class Boss:
    def __init__(self, kind, x, y, arena_center, arena_r):
        ...
        self.radius = 0.55 if kind != 'hound' else 0.4
        self.fx, self.fy = 0.0, 1.0
        self.alive = True
        self.active = False         # becomes True when the fight starts
        self.state = 'idle'
        self.timer = 0.0
        self.combo = 0
        self.anim_t = 0.0
        ...
        if self.state == 'idle':
            ...
            if d > 1.7:
                ...
                if random.random() < 0.5:
                    self.state = 'windup'
                    self.timer = 0.55 if not self.phase2 else 0.42
                    self.combo = 0
                else:
                    self.state = 'slam_wind'
                    self.timer = 0.8
            ...
            if self.timer <= 0:
                self.state = 'recover'
                self.timer = 1.1
                if self.phase2:   # shockwave ring of bolts
                    for i in range(8):
                        a = i * math.pi / 4
    ...
    def _ai_chorister(self, dt, world, player, projectiles, particles, enemies):
        ...
        if self.state == 'idle':
            ...
            if self.timer <= 0 and self.combo > 0:
                ...
                for i in range(n):
                    a = off + i * 2 * math.pi / n
                    projectiles.append(Projectile(self.x, self.y - 0.01,
    ...
    def _ai_hound(self, dt, world, player, projectiles, particles, enemies):
        ...
        if self.state == 'idle':
            ...
            if self.timer <= 0:
                self.state = 'circle'
                self.timer = random.uniform(0.9, 1.8)
        ...
        if self.state == 'idle':
            ...
            if self.timer <= 0:
                ...
                if d < 2.4:
                    ...
                    # 'rewrite': delayed AOEs stamped on the player's path
    ...
    def anim_frame(self):
        if self.state in ('windup', 'charge', 'lunge', 'burst', 'fan', 'slam_wind'):
            return 3
